...
bootstrap.create_tick_handler(function()
  ...
  if entities and entities.reactive_entity then
    ...
    inductor.enable(enable_control ~= 0);
    ...
    if enable_control ~= 0 then
      ...
      local data = __get_total_consume(surface_facility.get())
      local current_tick_consume = data - entities.reactive_entity.power_usage;
      ...
      entities.reactive_entity.power_usage = 0
      entities.pc_entity.electric_buffer_size = 0
      power_converter.set_signal(power_converter.output_type.output_power, 1, 0)
      power_converter.set_signal(power_converter.output_type.current_power, 2, 0)
    end
  end
end)